...
let check_comparable loc prim ty1 ty2 =
  if not (Liquid.DTypes.is_comparable ty1 && Liquid.DTypes.eq ty1 ty2) then
...
let find_var ?(count_used=true) env loc name =
  try
    ...
    { (Liquid.Expr.mk (Var name) ~loc ty) with effect }
  with Not_found ->
...
let eq_exp env (e1 : typed_exp) (e2 : typed_exp) =
  let eq_var v1 v2 =
    ...
    get v1 = get v2 in
  Liquid.Expr.eq_typed eq_var e1 e2
...
let error_prim loc prim args expected_args =
  let prim = Liquid.Prims.to_string prim in
...
let sig_of_contract contract =
  let c_sig = Liquid.Contracts.to_sig contract in
  let sig_name = StringMap.fold (fun name c_sig' -> function
      | Some _ as acc -> acc
      | None -> if Liquid.Contracts.eq_sig c_sig' c_sig then Some name else None
...
let rec typecheck env ( exp : syntax_exp ) : typed_exp =
  ...
  | Const { ty; const } ->
    Liquid.Expr.mk ?name:exp.name ~loc (Const { ty; const }) (ty:datatype)
  ...
  | Let { bnd_var; inline; bnd_val; body } ->
    ...
    Liquid.Expr.mk ?name:exp.name ~loc desc body.ty
  ...
  | Project { field; record } ->
    ...
    let ty = match record.ty with
      ...
      | rty -> error loc "not a record type: %s, has no field %s"
                 ...
                 field
    in
    Liquid.Expr.mk ?name:exp.name ~loc (Project { field; record }) ty

  | SetField { record; field; set_val } ->
    let record = typecheck env record in
    let exp_ty =
      let ty_name, _, ty =
        try Liquid.Env.Find.label field env.env
        with Not_found -> error loc "unbound record field %S" field
      in
      let record_ty = Liquid.Env.Find.dtype ty_name env.env in
      if not @@ Liquid.DTypes.eq record.ty record_ty then
    ...
    Liquid.Expr.mk ?name:exp.name ~loc (SetField { record; field; set_val }) record.ty
  ...
  | Seq (exp1, exp2) ->
    ...
    Liquid.Expr.mk ?name:exp.name ~loc desc exp2.ty
  ...
  | If { cond; ifthen; ifelse } ->
    ...
    Liquid.Expr.mk ?name:exp.name ~loc desc ty
  ...
  | Transfer { dest; amount } ->
    ...
    Liquid.Expr.mk ?name:exp.name ~loc desc Toperation
  ...
  | Apply { prim = Prim_unknown;
    ...
    ->
    typecheck env
      (Liquid.Expr.mk (Call { contract; amount; entry = Some entry; arg = param })
         ~loc ())
  ...
  | Apply { prim = Prim_unknown;
    ...
    let atys = List.map (Liquid.DTypes.tv_subst tsubst) eprim.atys in
    let rty = Liquid.DTypes.tv_subst tsubst eprim.rty in
    ...
    begin try List.iter2 (fun a aty ->
      if not (Liquid.DTypes.eq a.ty aty) then
        error loc "Bad %d args for primitive %S:\n    %s\n"
          (List.length args) (Liquid.Prims.to_string prim)
          (String.concat "\n    " (List.map (fun arg ->
    ...
    Liquid.Expr.mk ?name:exp.name ~loc (Apply { prim; args }) rty
  ...
  | Failwith arg ->
    ...
    Liquid.Expr.mk (Failwith arg) ~loc Tfail (* no name *)
  ...
  | MatchOption { arg; ifnone; some_name; ifsome } ->
    ...
    let ty =
      ...
      | ty1, ty2 ->
        if not @@ Liquid.DTypes.eq ty1 ty2 then
  ...
  | Loop { arg_name; body; arg } ->
    ...
    Liquid.Expr.mk ?name:exp.name ~loc (Loop { arg_name; body; arg }) arg.ty
  ...
  | LoopLeft { arg_name; body; arg; acc = Some acc } ->
    ...
    let res_ty = match body.ty with
      | Ttuple [Tor (left_ty, right_ty); acc_ty] ->
        if not @@ Liquid.DTypes.eq acc_ty acc.ty then
          error acc.loc
        ...
        if not @@ Liquid.DTypes.eq left_ty arg.ty then
          error arg.loc
    ...
    check_used env arg_name count;
    Liquid.Expr.mk ?name:exp.name ~loc (LoopLeft { arg_name; body; arg; acc = Some acc })
      ...
      | _ ->
        error loc
          "Loop.left body must be of type ('a, 'b) variant, \
           got %s instead" (LiquidPrinter.Liquid.string_of_type body.ty) in
    check_used env arg_name count;
    Liquid.Expr.mk ?name:exp.name ~loc (LoopLeft { arg_name; body; arg; acc = None }) res_ty
    ...
    let prim, arg_ty = match prim, arg.ty, acc.ty with
      ...
      | _ ->
        error arg.loc "%s expects a collection, got %s"
          (Liquid.Prims.Fold.to_string prim)
          (LiquidPrinter.Liquid.string_of_type arg.ty)
    in
    let (env, count) = new_binding env arg_name.nname arg_ty in
    let body = typecheck_expected
        (Liquid.Prims.Fold.to_string prim ^" body") env acc.ty body in
    check_used env arg_name count;
    Liquid.Expr.mk ?name:exp.name ~loc (Fold { prim; arg_name; body; arg; acc }) acc.ty
  ...
  | Map { prim; arg_name; body; arg } ->
    ...
    let prim, arg_ty = match prim, arg.ty with
      ...
      | _ ->
        error arg.loc "%s expects a collection, got %s"
          (Liquid.Prims.Map.to_string prim)
          (LiquidPrinter.Liquid.string_of_type arg.ty)
    in
    let (env, count) = new_binding env arg_name.nname arg_ty in
    let body = typecheck env body in
    ...
    in
    check_used env arg_name count;
    Liquid.Expr.mk ?name:exp.name ~loc (Map { prim; arg_name; body; arg }) ret_ty
  ...
  | MapFold { prim; arg_name; body; arg; acc } ->
    ...
    let prim, arg_ty = match prim, arg.ty, acc.ty with
      ...
      | _ ->
        error arg.loc "%s expects a collection, got %s"
          (Liquid.Prims.Map.Fold.to_string prim)
          (LiquidPrinter.Liquid.string_of_type arg.ty)
    in
    let (env, count) = new_binding env arg_name.nname arg_ty in
    let body = typecheck env body in
    let body_r = match body.ty with
      | Ttuple [r; baccty] when Liquid.DTypes.eq baccty acc.ty -> r
      | _ ->
        error body.loc
    ...
    in
    check_used env arg_name count;
    Liquid.Expr.mk ?name:exp.name ~loc (MapFold { prim; arg_name; body; arg; acc })
  ...
  | Record (( (label, _) :: _ ) as lab_x_exp_list) ->
    let ty_name, _, _ =
      try Liquid.Env.Find.label label env.env
    ...
    let fields = List.map (fun (label, exp) ->
        let ty_name', _, ty = try
            Liquid.Env.Find.label label env.env
          with Not_found -> error loc "unbound label %S" label
        in
    ...
    Liquid.Expr.mk ?name:exp.name ~loc (Record fields) record_ty
  ...
  (* TODO
     | Constructor(loc, Constr constr, arg)
      ...
      Liquid.Expr.mk ?name:exp.name ~loc (Constructor(loc, Constr constr, arg)) constr_ty
  *)
  ...
  | Constructor { constr = Constr constr; arg } ->
    begin try
        ...
        Liquid.Expr.mk ?name:exp.name ~loc (Constructor { constr = Constr constr; arg })
          constr_ty
      with Not_found ->
  ...
  | Constructor { constr = Left right_ty; arg } ->
    ...
    Liquid.Expr.mk ?name:exp.name ~loc (Constructor { constr = Left right_ty; arg }) ty

  | Constructor { constr = Right left_ty; arg } ->
    let arg = typecheck env arg in
    let ty = Tor (left_ty, arg.ty) in
    Liquid.Expr.mk ?name:exp.name ~loc (Constructor { constr = Right left_ty; arg }) ty
      ...
      | None ->
        ...
        let cases = List.map (fun (pat, e) ->
            ...
            let env, count_opt =
              ...
              | CConstr (constr, vars) ->
                let ty_name', var_ty =
                  try Liquid.Env.Find.constr constr env.env
  ...
  | CreateContract { args; contract } ->
    ...
    | [manager; delegate; spendable; delegatable; init_balance; init_storage] ->
      ...
      let init_storage = typecheck_expected "initial storage"
          env (Liquid.DTypes.lift contract.ty_env contract.storage) init_storage in
      let desc = CreateContract {
          args = [manager; delegate; spendable;
                  delegatable; init_balance; init_storage];
          contract } in
      Liquid.Expr.mk ?name:exp.name ~loc desc (Ttuple [Toperation; Taddress])
    | _ ->
...
and typecheck_prim1 env prim loc args =
  ...
  | Prim_tuple_set, [{ ty = tuple_ty };
    ...
    let ty = if not (Liquid.DTypes.eq ty expected_ty || ty = Tfail) then
...
and typecheck_prim2 env prim loc args =
  ...
  | Prim_map_find,
    ...
    ->
    if not @@ Liquid.DTypes.eq expected_key_ty key_ty then
  ...
  | Prim_map_update,
    ...
    if not @@ Liquid.DTypes.eq expected_value_ty value_ty then
  ...
  | Prim_self, [ Tunit ] -> Tcontract (Liquid.Contracts.sig_of_full_sig env.t_contract_sig)
  ...
  | Prim_default_account, [ Tkey_hash ] ->
    Tcontract Liquid.Contracts.unit_sig
  ...
  | Prim_exec, [ ty;
    ...
    if not @@ Liquid.DTypes.eq ty from_ty then
  ...
  | Prim_Cons, [ head_ty; Tlist tail_ty ] ->
    if not @@ Liquid.DTypes.eq head_ty tail_ty then
...
and typecheck_expected info env expected_ty exp =
  ...
  if not @@ Liquid.DTypes.eq exp.ty expected_ty && exp.ty <> Tfail then
    type_error exp.loc
...
and typecheck_apply ?name env prim loc args =
  ...
  Liquid.Expr.mk ?name (Apply { prim; args }) ty
...
let rec type_of_const = function
  ...
  | CContract _ -> Tcontract Liquid.Contracts.unit_sig
