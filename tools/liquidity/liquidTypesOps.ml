...
let entry_prefix = "_Liq_entry_"
(** Prefixes the name of an entry point with the appropriate prefix. *)
let prefix_entry s = entry_prefix ^ s

(** Prefix for constructor used to encode contracts *)
let contract_prefix = "_Liq_contract_"

let entry_name_of_case s =
  Scanf.sscanf s
    (Scanf.format_from_string entry_prefix "" ^^ "%s%!")
    (fun x -> x)
...
let contract_name_of_annot s =
  Scanf.sscanf s
    (Scanf.format_from_string contract_prefix "" ^^ "%s%!")
    (fun x -> x)
