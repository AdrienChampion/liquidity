  ...
  let ty = match expr with
    ...
    | Prim(_, "contract", [x], annots) ->
      ...
      let entries_sig = match parameter with
        | Tsum (_, constrs)
          when List.for_all (fun (s, _) -> Liquid.Idents.Entry.is_prefixed s) constrs ->
          List.map (fun (s, ty) ->
              { entry_name = Liquid.Idents.Entry.name_of_prefixed s;
                parameter = ty;
      ...
      begin match
          sig_name,
          List.find_opt (fun (n, c_sig') -> Liquid.Contracts.eq_sig c_sig c_sig')
            env.contract_types
        with
