...
let mk ?name ~loc (desc: (datatype, typed) exp_desc) ty = Liquid.Expr.mk ?name ~loc desc ty
...
let rec decode ( exp : encoded_exp ) : typed_exp =
  ...
  | Call { contract; amount; entry; arg } ->
    ...
    let desc = match entry, arg.desc with
      | None, Constructor { constr = Constr c; arg } when Liquid.Idents.Entry.is_prefixed c ->
        let entry = Some (Liquid.Idents.Entry.name_of_prefixed c) in
...
and entry_of_case param_constrs top_storage (pat, body) =
  ...
  | CConstr (s, [parameter_name]),
    Let { bnd_var = { nname = storage_name };
          ...
          body = code }
    when Liquid.Idents.Entry.is_prefixed s && var_storage = top_storage ->
    let entry_name = Liquid.Idents.Entry.name_of_prefixed s in
  ...
  | CConstr (s, [parameter_name]), _
    when Liquid.Idents.Entry.is_prefixed s ->
    let entry_name = Liquid.Idents.Entry.name_of_prefixed s in
    let parameter = List.assoc s param_constrs in
    {
      entry_sig = {
  ...
  | MatchVariant { arg = { desc = Var var_parameter}; cases }
    when var_parameter = top_parameter &&
         List.for_all (function
             | CConstr (s, _), _ -> Liquid.Idents.Entry.is_prefixed s
             | _ -> false) cases
    ->
