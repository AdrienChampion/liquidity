...
and convert_contract_sig ~abbrev csig =
  ...
  let typ = StringMap.fold (fun n csig' -> function
      ...
      | None ->
        ...
        | Some name when Liquid.DTypes.eq (Tcontract csig') (Tcontract csig) ->
...
let rec convert_code ~abbrev (expr : (datatype, 'a) exp) =
  ...
  | Fold { prim = (Prim_map_iter|Prim_set_iter|Prim_list_iter as prim);
    ...
    Exp.apply ~loc
      (Exp.ident (lid (Liquid.Prims.Fold.to_string prim)))
  ...
  | Map { prim; arg_name;
    ...
    Exp.apply ~loc
      (Exp.ident (lid (Liquid.Prims.Map.to_string prim)))
      [
        Nolabel, convert_code ~abbrev f;
        Nolabel, convert_code ~abbrev arg;
      ]
  ...
  | MapFold { prim; arg_name;
    ...
    Exp.apply ~loc
      (Exp.ident (lid (Liquid.Prims.Map.Fold.to_string prim)))
      [
        Nolabel, convert_code ~abbrev f;
        Nolabel, convert_code ~abbrev arg;
        Nolabel, convert_code ~abbrev acc;
